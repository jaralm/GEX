...
gex_calculator.py
-----------------
Calcula Gamma Exposure (GEX) y KPIs derivados desde el CSV de meff_opciones.py.

Parámetros del modelo:
  R            = 0.025  (BCE, tasa libre de riesgo)
  SWEEP_MARGIN = 0.15   (±15% del spot para buscar Zero Gamma)
...
def _calcular_zero_gamma(S0: float, df: pd.DataFrame, mult: int):
    ...
    """
    # Extraer arrays una sola vez — sin iterrows()
    K     = df["strike_pts"].values.astype(float)
    T     = df["T"].values.astype(float)
    sigma = df["sigma"].values.astype(float)
    ...
    # Para cada punto del barrido: gamma vectorizada → GEX neto
    gex_s = np.array([
    ...
    # Cruce más cercano al spot actual
...
def calcular_kpis(csv_path: str, accion: str = None,
    ...
    # Validación temprana: columnas requeridas
